...
pkgdatadir = $(datadir)/lava
pkgincludedir = $(includedir)/lava
pkglibdir = $(libdir)/lava
pkglibexecdir = $(libexecdir)/lava
...
CONFIG_HEADER = $(top_builddir)/src/config/lava-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/lava-config.h.in \
...
ACLOCAL = ${SHELL} /root/lava/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/lava/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/lava/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/lava/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx
BITCOIN_CLI_NAME = lava-cli
BITCOIN_DAEMON_NAME = lavad
BITCOIN_GUI_NAME = lava-qt
BITCOIN_TX_NAME = lava-tx
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/lava/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
OTOOL64 = 
PACKAGE = lava
PACKAGE_BUGREPORT = www.lava.org
PACKAGE_NAME = LAVA Core
PACKAGE_STRING = LAVA Core 4.0.0
PACKAGE_TARNAME = lava
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /root/lava
abs_srcdir = /root/lava
abs_top_builddir = /root/lava
abs_top_srcdir = /root/lava
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/lava/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_lava_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_lava_coverage.info test_lava.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/LAVA-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/lava-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/lava-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/lava-config.h
$(top_srcdir)/src/config/lava-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/lava-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/LAVA-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/LAVA-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/LAVA-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_lava.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_lava -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_lava_filtered.info: test_lava.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_lava_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_lava_coverage.info: baseline_filtered_combined.info test_lava_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_lava_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_lava_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_lava_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_lava.coverage/.dirstamp:  test_lava_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_lava.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_lava.coverage/ total.coverage/ $(OSX_APP)
