...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from LAVA
...
{
    RenameThread("lava-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZLAVAMinted()
{
...
void RecalculateZLAVASpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zLAVA supply
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert LAVA to zLAVA
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // LAVA: We use certain sporks during IBD, so check to see if they are
